...
@media(max-width:480px){.countdown{border-radius:22px}.countdownCounter{top:58%}.countdownCounter b{font-size:38px;-webkit-text-stroke:1px #e2a62d}.countdownCounter small{font-size:11px;margin-top:4px}}
</style></head>`);
        ...
        html = html.replace('</body>', `<script>(function(){const target=new Date(2027,5,26,0,0,0);function dayWord(n){const n100=n%100,n10=n%10;if(n100>=11&&n100<=14)return'дней';if(n10===1)return'день';if(n10>=2&&n10<=4)return'дня';return'дней'}function tick(){const diff=Math.max(0,target-new Date());const days=Math.floor(diff/86400000);const a=document.getElementById('cdDays'),w=document.getElementById('cdDayWord');if(a)a.textContent=days;if(w)w.textContent=dayWord(days)}tick();setInterval(tick,60000)})();</script></body>`);
