...
function paymentStatus(p){const a=accountOf(p),total=a.stages.reduce((s,x)=>s+x.amount,0);if(p.role==='teacher')return total?'Переплата':'Бесплатно';if(!p.payment&&p.paid)return 'Оплачено ранее';if(a.cost===null)return total?'Частично · стоимость не задана':'Стоимость не задана';if(total>a.cost)return 'Переплата';if(total===a.cost)return 'Оплачено';return total?'Частично':'Не оплачено'}
...
function editorPaymentSummary(p){return paymentSummary(p).replace('Стоимость:','Полная стоимость:').replace('Внесено:','Всего внесено:').replace('Осталось:','Осталось оплатить:')}
function accountFields(p){const a=accountOf(p);return `<div class="payment-account"><div class="field"><label>Полная стоимость, ₽ (по статусу)</label><input class="payment-cost" readonly inputmode="decimal" placeholder="Не задана" value="${a.cost===null?'':a.cost/100}" aria-label="Полная стоимость"></div>${!p.payment&&p.paid?'<div class="note">Ранее отмечено «Оплачено». Сумма неизвестна; внесите фактические платежи.</div>':''}${a.stages.map((x,i)=>`<div class="payment-stage"><b>${['Первый взнос','Второй взнос','Третий взнос'][i]}</b><div class="payment-inputs"><label>Внесено, ₽<input class="payment-amount" inputmode="decimal" value="${x.amount?x.amount/100:''}" placeholder="0" aria-label="${['Первый взнос','Второй взнос','Третий взнос'][i]}: сумма"></label><label>Дата платежа<input class="payment-date" type="date" value="${esc(x.date)}" aria-label="${['Первый взнос','Второй взнос','Третий взнос'][i]}: дата"></label></div></div>`).join('')}<div class="note payment-total">${esc(editorPaymentSummary(p))}</div></div>`}
...
function updatePaymentTotals(event){const root=event.target.closest('.payment-account');if(!root)return;try{root.querySelector('.payment-total').textContent=editorPaymentSummary({payment:readAccount(root)})}catch(e){root.querySelector('.payment-total').textContent=e.message}}
...
async function loadServer(){
 ...
 try{
  ...
  data=Object.fromEntries(['11А','11Б','11В'].map(c=>[c,j.data.classes[c].map(x=>({...x,role:x.role==='teacher'?'teacher':isCompanion(x)?'companion':'student'}))]));
...
const contractRefundAnnouncement={"infoOnly":true,"title":"Если передумали участвовать: возврат оплаты","visible":true,"updatedAt":"2026-09-14T00:00:00Z","text":"Сразу сообщите администратору своего класса. Возврат зависит от даты отказа, количества отказавшихся и расходов организатора.\n\nДО 15 АПРЕЛЯ 2027\nЕсли отказывается менее 10% платных участников всей группы, внесённые за них деньги полностью засчитываются в счёт следующих платежей по договору. Это не означает немедленный возврат семье.\nЕсли отказывается более 10% — предусмотрено удержание расходов, не менее 4 000 ₽ за каждого отказавшегося. Случай ровно 10% отдельно в договоре не описан — его нужно уточнять у организатора.\n\nПОСЛЕ 15 АПРЕЛЯ\nУдержание — не менее 4 000 ₽ с человека. Оно может быть больше.\n\nПОСЛЕ 15 МАЯ\nУдержание — не менее половины полной стоимости участия.\n\nЗА 10 РАБОЧИХ ДНЕЙ ДО ВЫПУСКНОГО И ПОЗДНЕЕ\nПо договору удерживается вся стоимость. Для подтверждённой болезни есть исключение: сокращение числа участников не менее чем за 5 рабочих дней, не более 5% платных участников, с подтверждающими документами. При этом расходы организатора также удерживаются.\n\nКАК ОФОРМЛЯЕТСЯ ВОЗВРАТ\nОбращайтесь к администратору класса. Представитель Заказчика подаёт организатору подписанное заявление. По договору возврат производится в течение 10 рабочих дней с даты получения и согласования заявления.\n\nТочную сумму согласовывают с организатором. Полный возврат заранее обещать нельзя. Неявка по договору не предусматривает возврата оплаты.\n\nПамятка по пунктам 2.7 и 5.3–5.9 договора №710. Полный отказ всей группы регулируется отдельно — разделом 6. Договор доступен в «Ещё» → «Договор и программа выпускного»."};
function currentAnnouncement(){const a=paymentPayload.announcement;return a&&typeof a==='object'?a:contractRefundAnnouncement}
function renderAnnouncement(){
 const a=currentAnnouncement(),visible=!!(a&&a.visible&&a.title);
 const card=document.getElementById('pinned-announcement');card.hidden=!visible||a?.infoOnly===true;
...
async function saveAnnouncement(event){
 ...
 try{
  paymentPayload={...paymentPayload,announcement:{title,text,visible,infoOnly:currentAnnouncement()?.infoOnly===true,updatedAt:new Date().toISOString()}};
  await saveData();
...
function expenseTotals(list){
 return list.reduce((s,e)=>{const plan=expenseKopecks(e.amount),paid=expenseKopecks(e.paidAmount);s.plan+=plan;s.paid+=paid;s.due+=Math.max(0,plan-paid);s.over+=e.amount==null?0:Math.max(0,paid-plan);return s},{plan:0,paid:0,due:0,over:0});
}
renderExpenses=function(){
 const total=expenseTotals(expenses),canEdit=admin&&verifiedAdmin&&serverReady,unknown=expenses.some(e=>e.amount==null);
 let h='<div class="payment-summary-card"><h3>Дополнительные расходы</h3><dl><div><dt>План</dt><dd>'+(unknown?'Уточняется':rub(total.plan))+'</dd></div><div><dt>Оплачено</dt><dd>'+rub(total.paid)+'</dd></div><div><dt>Осталось</dt><dd>'+(unknown?'Уточняется':rub(total.due))+'</dd></div></dl><p>Оплаты подрядчикам и поставщикам. Взносы родителей учитываются в разделе «Статус оплаты».</p>'+(total.over?'<p>Сверх плана: '+rub(total.over)+'</p>':'')+'</div>';

 expenses.forEach((e,i)=>{const plan=expenseKopecks(e.amount),paid=expenseKopecks(e.paidAmount);h+='<div class="row"><div class="grow"><b>'+esc(e.name)+'</b><small>План: '+(e.amount==null?'Уточняется':rub(plan))+'</small><small>Оплачено: '+rub(paid)+'</small><small>'+(e.amount==null?'Осталось: уточняется':paid>plan?'Сверх плана: '+rub(paid-plan):'Осталось: '+rub(plan-paid))+'</small>'+(e.note?'<small class="expense-note">'+esc(e.note)+'</small>':'')+'</div>'+(canEdit?'<button type="button" class="editbtn" onclick="editExpense('+i+')">Изменить</button>':'')+'</div>'});
...
async function saveExpense(event){
 ...
 try{
  ...
  const planValue=moneyInput(document.getElementById('expense-plan').value,true),amount=planValue===null?null:planValue/100,paidAmount=moneyInput(document.getElementById('expense-paid').value)/100;
...
/* Keep payment headings tied to the person being edited. */
function updatePaymentPersonTitle(){
 const title=document.querySelector('#paymentField .payment-person-title');if(!title)return;
 const name=document.getElementById('nameInput').value.trim();
 const value=document.getElementById('roleInput').value;const role=value==='teacher'?'преподавателя':value==='companion'?'сопровождающего':'выпускника';
 title.textContent='Оплата за '+role+(name?' — '+name:'');
}
document.getElementById('nameInput').addEventListener('input',updatePaymentPersonTitle);
document.getElementById('guestFields').addEventListener('input',event=>{
 if(!event.target.matches('.guest-name'))return;
 const name=event.target.value.trim();
 event.target.closest('.payment-guest').querySelector('summary').textContent='Оплата за сопровождающего'+(name?' — '+name:'');
});

/* Full price follows participant role; actual contributions are unchanged. */
function syncRolePriceField(){
 const input=document.querySelector('#paymentField .payment-cost');if(!input)return;
 const role=document.getElementById('roleInput').value;
 input.value=role==='teacher'?0:role==='companion'?21000:24400;
 updatePaymentTotals({target:input});
}
...
startSession=async function(){
 ...
 try{
 const r=await fetch('/api/role-prices',{method:'POST',signal:controller.signal,headers:{'x-telegram-init-data':tg.initData}});
 const j=await r.json();if(!r.ok||!j.ok)throw Error(j.error||'Не удалось применить стоимость.');
 if(j.changed)await loadServer();
 }catch(e){alert('Не удалось подтвердить обновление стоимости. Откройте приложение заново: '+e.message)}finally{clearTimeout(timer)}
};

/* One-time replacement of the untouched starter expense list. */
const sessionBeforeExpenseSetup=startSession;
startSession=async function(){
 await sessionBeforeExpenseSetup();if(!verifiedAdmin||!tg?.initData)return;
 const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),20000);
 try{
  const r=await fetch('/api/expense-setup',{method:'POST',signal:controller.signal,headers:{'x-telegram-init-data':tg.initData}});
  const j=await r.json();if(!r.ok||!j.ok)throw Error(j.error||'Ошибка обновления сметы.');
  if(j.changed)await loadServer();
