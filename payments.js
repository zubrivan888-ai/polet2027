...
function syncRoleFields(){const companion=document.getElementById('roleInput').value==='companion';document.getElementById('guestFields').style.display=companion?'none':'block';document.querySelector('#modal .mini').style.display=companion?'none':'inline-block';updatePaymentPersonTitle()}
function openEditor(c,i){if(!admin||!verifiedAdmin||!serverReady)return alert('Редактирование доступно администратору после загрузки общей базы.');editing=c!=null?{c,i}:null;const p=editing?data[c][i]:{name:'',guests:[],paid:false,role:'student'};document.getElementById('editorTitle').textContent=editing?'Участник и платежи':'Новый участник';document.getElementById('classInput').value=c||'11А';document.getElementById('nameInput').value=p.name||'';document.getElementById('roleInput').value=isCompanion(p)?'companion':'student';document.getElementById('paymentField').innerHTML='<h4 class="payment-person-title"></h4>'+accountFields(p);document.getElementById('guestFields').innerHTML='';(p.guests||[]).forEach((g,i)=>addGuestField(g,p.guestPayments?.[i]||{}));document.getElementById('deleteBtn').style.display=editing?'block':'none';syncRoleFields();document.getElementById('modal').classList.add('open')}
function addGuestField(v='',old={}){const d=document.createElement('div');d.className='guestline payment-guest';d.paymentRecord=old;d.innerHTML=`<div class="guest-heading"><input class="guest-name" placeholder="Например: мама, папа" aria-label="Имя сопровождающего" value="${esc(v)}"><button type="button" onclick="if(confirm('Удалить гостя вместе с его платежами?'))this.closest('.payment-guest').remove()">×</button></div><details><summary>Оплата за сопровождающего${v?' — '+esc(v):''}</summary>${accountFields(old)}</details>`;document.getElementById('guestFields').appendChild(d)}
...
function updatePaymentPersonTitle(){
 ...
 const role=document.getElementById('roleInput').value==='companion'?'сопровождающего':'выпускника';
